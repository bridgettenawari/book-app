...
with app.app_context():
    ...
    # Fetch Global books
    global_books_data = fetch_books_from_openlibrary("fall", limit=300, country="Global")
    global_books = []
    for b in global_books_data:
        ...
        new_book = Book(
            ...
            series_name=json.dumps(b.get("series_name")) if b.get("series_name") else None,  # FIXED
    ...
    notes = [
        ...
        Note(content="This Kenyan book has amazing cultural insights.", book_id=kenyan_books[0].id, user_id=users[1].id),
    ]
    db.session.add_all(notes)
    db.session.commit()

    print("Database seeded successfully with EPUB links!")
